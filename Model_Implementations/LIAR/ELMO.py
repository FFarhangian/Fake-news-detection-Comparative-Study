...
df1_train_seq = torch.tensor(df1_tokens_train['token_ids'])
df1_train_labels = torch.tensor(df1_train_class.tolist())

df1_test_seq = torch.tensor(df1_tokens_test['token_ids'])
df1_test_labels = torch.tensor(df1_test_class.tolist())

# Create data loader
dataset = TensorDataset(df1_train_seq, df1_train_labels)
...
for fold, (train_idx, val_idx) in enumerate(skf.split(df1_train_seq, df1_train_labels)):
    ...
    test_dataset = TensorDataset(df1_test_seq, df1_test_labels)
...
np.savetxt("df1_ELMO_preds.csv", test_preds, delimiter=",")
