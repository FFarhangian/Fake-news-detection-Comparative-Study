...
xlnet = AutoModel.from_pretrained(xlnet_model_name)
tokenizer = AutoTokenizer.from_pretrained(xlnet_model_name)
...
class XLNET_Arch(nn.Module):
    ...
    def forward(self, sent_id, mask):
        cls_hs,= self.xlnet(sent_id, attention_mask=mask)
...
df1_tokens_train = tokenize_sequences(df1_train.tolist())
df1_tokens_test = tokenize_sequences(df1_test.tolist())

# Convert the encodings to PyTorch tensors
df1_train_seq = torch.tensor(df1_tokens_train['input_ids'])
df1_train_mask = torch.tensor(df1_tokens_train['attention_mask'])
df1_train_labels = torch.tensor(df1_train_class.tolist())

df1_test_seq = torch.tensor(df1_tokens_test['input_ids'])
df1_test_mask = torch.tensor(df1_tokens_test['attention_mask'])
df1_test_labels = torch.tensor(df1_test_class.tolist())

# Create data loader
dataset = TensorDataset(df1_train_seq, df1_train_mask, df1_train_labels)
...
for fold, (train_idx, val_idx) in enumerate(skf.split(df1_train_seq, df1_train_labels)):
    ...
    test_dataset = TensorDataset(df1_test_seq, df1_test_mask, df1_test_labels)
    ...
    test_report = classification_report(df1_test_labels.tolist(), test_preds)
    print(f"Test Report:\n{test_report}\n")
...
np.savetxt("df1_XLNET_preds.csv", test_preds, delimiter=",")
