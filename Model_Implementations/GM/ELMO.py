...
df4_train_seq = torch.tensor(df4_tokens_train['token_ids'])
df4_train_labels = torch.tensor(df4_train_class.tolist())

df4_test_seq = torch.tensor(df4_tokens_test['token_ids'])
df4_test_labels = torch.tensor(df4_test_class.tolist())

# Create data loader
dataset = TensorDataset(df4_train_seq, df4_train_labels)
...
for fold, (train_idx, val_idx) in enumerate(skf.split(df4_train_seq, df4_train_labels)):
    ...
    test_dataset = TensorDataset(df4_test_seq, df4_test_labels)
...
np.savetxt("df4_ELMO_preds.csv", test_preds, delimiter=",")
