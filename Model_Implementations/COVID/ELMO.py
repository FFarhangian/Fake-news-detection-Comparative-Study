...
df3_train_seq = torch.tensor(df3_tokens_train['token_ids'])
df3_train_labels = torch.tensor(df3_train_class.tolist())

df3_test_seq = torch.tensor(df3_tokens_test['token_ids'])
df3_test_labels = torch.tensor(df3_test_class.tolist())

# Create data loader
dataset = TensorDataset(df3_train_seq, df3_train_labels)
...
for fold, (train_idx, val_idx) in enumerate(skf.split(df3_train_seq, df3_train_labels)):
    ...
    test_dataset = TensorDataset(df3_test_seq, df3_test_labels)
...
np.savetxt("df3_ELMO_preds.csv", test_preds, delimiter=",")
