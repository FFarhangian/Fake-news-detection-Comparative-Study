...
df2_train_seq = torch.tensor(df2_tokens_train['input_ids'])
df2_train_mask = torch.tensor(df2_tokens_train['attention_mask'])
df2_train_labels = torch.tensor(df2_train_class.tolist())

df2_test_seq = torch.tensor(df2_tokens_test['input_ids'])
df2_test_mask = torch.tensor(df2_tokens_test['attention_mask'])
df2_test_labels = torch.tensor(df2_test_class.tolist())

# Create data loader
dataset = TensorDataset(df2_train_seq, df2_train_mask, df2_train_labels)
...
for fold, (train_idx, val_idx) in enumerate(skf.split(df2_train_seq, df2_train_labels)):
    ...
    test_dataset = TensorDataset(df2_test_seq, df2_test_mask, df2_test_labels)
...
np.savetxt("df2_ELECTRA_preds.csv", test_preds, delimiter=",")
